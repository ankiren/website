...
import { getEinsteinSkills, getEinsteinLifeAspects, UserCustomSkill } from "../../_buffettSkills";

const PERSON_AGE = 20;
const PAGE_TITLE = "Albert Einstein Age 20 (1899)";
const PAGE_DESC = "A struggling student with revolutionary ideas brewing";
...
const initialUserSkills = getEinsteinSkills(PERSON_AGE);
const lifeAspects = getEinsteinLifeAspects(PERSON_AGE);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-violet-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          <div className="relative"><button onClick={() => setShowSortDropdown(!showSortDropdown)} className="px-4 py-3 bg-white border border-gray-200 rounded-xl flex items-center gap-2 shadow-sm"><Filter className="w-5 h-5" />Sort: {sortBy}<ChevronDown className="w-4 h-4" /></button>{showSortDropdown && <div className="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-xl border py-1 min-w-[140px] z-10">{['recent', 'name', 'score'].map(v => <button key={v} onClick={() => { setSortBy(v as typeof sortBy); setShowSortDropdown(false); }} className={`w-full px-4 py-2 text-left text-sm ${sortBy === v ? 'text-violet-600 bg-violet-50' : ''}`}>{v}</button>)}</div>}</div>
          <div className="flex bg-white border border-gray-200 rounded-xl p-1 shadow-sm"><button onClick={() => setViewMode('grid')} className={`px-4 py-2 rounded-lg ${viewMode === 'grid' ? 'bg-violet-100 text-violet-700' : 'text-gray-500'}`}><LayoutGrid className="w-5 h-5" /></button><button onClick={() => setViewMode('list')} className={`px-4 py-2 rounded-lg ${viewMode === 'list' ? 'bg-violet-100 text-violet-700' : 'text-gray-500'}`}><List className="w-5 h-5" /></button></div>
        </div>
