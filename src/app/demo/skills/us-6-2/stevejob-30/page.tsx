...
import { Plus, Search, Sparkles, ChevronDown, Check, BookOpen, Edit2, Trash2, MoreHorizontal, GraduationCap, Briefcase, FolderKanban, Award } from "lucide-react";
import { Modal } from "@/components/ui/Modal";
import Button from "@/components/ui/Button";
import Input from "@/components/ui/Input";
import { DemoHeader, SkillIcon, getScoreLevel, RadarChart } from "../../_shared";
import { getSteveJobsSkills, getSteveJobsLifeAspects, getSteveJobsCourses, getSteveJobsRoles, getSteveJobsProjects, getSteveJobsCertificates, UserCustomSkill, Course, Role, Project, Certificate } from "../../_buffettSkills";

const PERSON_AGE = 30;
const PAGE_TITLE = "Steve Jobs Age 30 (1985)";
const PAGE_DESC = "Visionary leader building the future of personal computing";
...
const initialUserSkills = getSteveJobsSkills(PERSON_AGE);
const lifeAspects = getSteveJobsLifeAspects(PERSON_AGE);
const courses = getSteveJobsCourses(PERSON_AGE);
const roles = getSteveJobsRoles(PERSON_AGE);
const projects = getSteveJobsProjects(PERSON_AGE);
const certificates = getSteveJobsCertificates(PERSON_AGE);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-orange-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="relative overflow-hidden bg-gradient-to-br from-orange-500 via-amber-500 to-yellow-500 rounded-3xl p-8 mb-8">
          <div className="absolute top-0 right-0 w-96 h-96 bg-white/10 rounded-full -translate-y-1/2 translate-x-1/2" />
          <div className="relative">
            <div className="flex items-start justify-between"><div><h2 className="text-3xl font-bold text-white mb-2">{PAGE_TITLE}</h2><p className="text-orange-100">{PAGE_DESC}</p></div><button onClick={() => { setEditingSkill(null); setShowAddModal(true); }} className="px-5 py-3 bg-white text-orange-600 font-semibold rounded-xl shadow-lg hover:scale-105 transition-all flex items-center gap-2"><Plus className="w-5 h-5" />Add Skill</button></div>
          </div>
        </div>
                ...
                {lifeAspects.aspects.map((aspect, i) => (
                  <div key={i} className="bg-gray-50 rounded-xl p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <SkillIcon name={aspect.icon} className="w-4 h-4 text-orange-600" />
                      <span className="text-sm font-medium text-gray-700">{aspect.name}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-orange-500 to-amber-500 rounded-full" style={{ width: `${aspect.value}%` }} />
                      </div>
                      <span className="text-sm font-bold text-orange-600">{aspect.value}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-2 mb-6">
          <div className="flex gap-2 overflow-x-auto">
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id as typeof activeTab)} className={`flex items-center gap-2 px-4 py-3 rounded-xl font-medium transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-orange-500 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
        ...
        {activeTab === "skills" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {skills.map(skill => <SkillCard key={skill.id} skill={skill} onEdit={s => { setEditingSkill(s); setShowAddModal(true); }} onDelete={id => confirm("Delete?") && setSkills(p => p.filter(s => s.id !== id))} />)}
          </div>
        )}
        {activeTab === "courses" && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{courses.map(course => <CourseCard key={course.id} course={course} />)}</div>}
        {activeTab === "roles" && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{roles.map(role => <RoleCard key={role.id} role={role} />)}</div>}
        {activeTab === "projects" && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{projects.map(project => <ProjectCard key={project.id} project={project} />)}</div>}
        {activeTab === "certificates" && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{certificates.map(certificate => <CertificateCard key={certificate.id} certificate={certificate} />)}</div>}
      </main>
