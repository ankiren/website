...
import { getMozartSkills, getMozartLifeAspects, UserCustomSkill } from "../../_buffettSkills";

const PERSON_AGE = 50;
const PAGE_TITLE = "Mozart Age 50 (1806) - Hypothetical";
const PAGE_DESC = "What if Mozart had lived? Revered master shaping European music";
const THEME_COLOR = "rose";
...
const initialUserSkills = getMozartSkills(PERSON_AGE);
const lifeAspects = getMozartLifeAspects(PERSON_AGE);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-rose-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="relative overflow-hidden bg-gradient-to-br from-rose-500 via-pink-500 to-fuchsia-500 rounded-3xl p-8 mb-8">
          <div className="absolute top-0 right-0 w-96 h-96 bg-white/10 rounded-full -translate-y-1/2 translate-x-1/2" />
          <div className="relative">
            <div className="flex items-start justify-between mb-6"><div><h2 className="text-3xl font-bold text-white mb-2">{PAGE_TITLE}</h2><p className="text-rose-100">{PAGE_DESC}</p></div><button onClick={() => { setEditingSkill(null); setShowAddModal(true); }} className="px-5 py-3 bg-white text-rose-600 font-semibold rounded-xl shadow-lg hover:scale-105 transition-all flex items-center gap-2"><Plus className="w-5 h-5" />Add</button></div>
                ...
                {lifeAspects.aspects.map((aspect, i) => (
                  <div key={i} className="bg-gray-50 rounded-xl p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <SkillIcon name={aspect.icon} className="w-4 h-4 text-rose-600" />
                      <span className="text-sm font-medium text-gray-700">{aspect.name}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-rose-500 to-pink-500 rounded-full" style={{ width: `${aspect.value}%` }} />
                      </div>
                      <span className="text-sm font-bold text-rose-600">{aspect.value}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          <div className="relative"><button onClick={() => setShowSortDropdown(!showSortDropdown)} className="px-4 py-3 bg-white border border-gray-200 rounded-xl flex items-center gap-2 shadow-sm"><Filter className="w-5 h-5" />Sort: {sortBy}<ChevronDown className="w-4 h-4" /></button>{showSortDropdown && <div className="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-xl border py-1 min-w-[140px] z-10">{['recent', 'name', 'score'].map(v => <button key={v} onClick={() => { setSortBy(v as typeof sortBy); setShowSortDropdown(false); }} className={`w-full px-4 py-2 text-left text-sm ${sortBy === v ? 'text-rose-600 bg-rose-50' : ''}`}>{v}</button>)}</div>}</div>
          <div className="flex bg-white border border-gray-200 rounded-xl p-1 shadow-sm"><button onClick={() => setViewMode('grid')} className={`px-4 py-2 rounded-lg ${viewMode === 'grid' ? 'bg-rose-100 text-rose-700' : 'text-gray-500'}`}><LayoutGrid className="w-5 h-5" /></button><button onClick={() => setViewMode('list')} className={`px-4 py-2 rounded-lg ${viewMode === 'list' ? 'bg-rose-100 text-rose-700' : 'text-gray-500'}`}><List className="w-5 h-5" /></button></div>
        </div>
