...
const PERSON_AGE = 10;
const PAGE_TITLE = "Buffett Age 10 (1940)";
const PAGE_DESC = "Young Warren's early interests";
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
                ...
                {lifeAspects.aspects.map((aspect, i) => (
                  <div key={i} className="bg-gray-50 rounded-xl p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <SkillIcon name={aspect.icon} className="w-4 h-4 text-violet-600" />
                      <span className="text-sm font-medium text-gray-700">{aspect.name}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-violet-500 to-purple-500 rounded-full" style={{ width: `${aspect.value}%` }} />
                      </div>
                      <span className="text-sm font-bold text-violet-600">{aspect.value}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-2 mb-6">
          <div className="flex gap-2 overflow-x-auto">
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id as typeof activeTab)} className={`flex items-center gap-2 px-4 py-3 rounded-xl font-medium transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-violet-600 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
